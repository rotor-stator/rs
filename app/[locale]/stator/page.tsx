...
import { statorMaterials } from "@/data/materials";
...
type ProductsData = Record<
  string,
  Record<string, { stator: Omit<Product, "id" | "manufacturer" | "model" | "category">[] }>
>;
...
function StatorPageContent() {
  ...
  const products: Product[] = (() => {
    ...
    const raw = data[selectedMfr]?.[selectedModel]?.stator ?? [];
    return raw.map((p) => ({
      ...p,
      id: p.partNumber,
